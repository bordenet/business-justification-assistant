/**
 * Business Justification Validator - Scoring Logic (Inline Version)
 *
 * UNIFIED 4-PILLAR TAXONOMY (aligned with validator.js and prompts.js):
...
const STAKEHOLDER_PATTERNS = EXECUTION_PATTERNS;
...
// Legacy detection functions using inline regex patterns for backward compatibility
...
export function detectMeasurableGoals(text) {
  ...
  return {
    ...
    hasQuantified: measurableMatches.length > 0,
    ...
    indicators: [
      ...
      measurableMatches.length > 0 && 'Quantified metrics present'
    ].filter(Boolean)
  };
}
...
export function detectStakeholders(text) {
  const stakeholderMatches = text.match(/\b(stakeholder|owner|lead|team|responsible|accountable|raci|sponsor|approver)\b/gi) || [];
  const roleMatches = text.match(/\b(responsible|accountable|consulted|informed|raci)\b/gi) || [];
  const hasStakeholderSection = EXECUTION_PATTERNS.stakeholderSection.test(text);
